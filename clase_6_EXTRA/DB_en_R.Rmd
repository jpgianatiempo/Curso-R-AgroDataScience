...
variables_planteo <- c("Adopción de NT","max_min","nivel","id_campaña","id_grano","id_zona",
                       "descr_grano","descr_campania","Semilla","Urea","PDA","Glifosato concentrado - Barbecho",
                       "Glifosato concentrado - cultivo","Hib. RR Bt2","2-4D","Dicamba","Metsulfurón",
...
planteos <- Tipo_caracteristicas_retaa %>%  
  ...
  as_tibble() %>% 
  #Calculo promedio por campaña, nivel, zona, insumo y grano
  group_by( id_campaña ,descripcion_campania , nivel, id_zona, descripcion_zona_retaa,
            id_grano, descripcion_grano, descripcion ) %>%  
  summarise(promedio_max = mean(max, na.rm = T), #proemdio maximo aplicado
  ...
  mutate(promedio = (promedio_min+promedio_max)/2, #calculo promedio
         promedio = ifelse(is.na(promedio),0,promedio)
         ) %>% 
   select(-c(promedio_max,promedio_min,promedio_porc_aplic) )  
  

#Funcion para escalar el nivel de adopcion tecnologica
escalar <- function(x){ x = x/sum(x,na.rm = T) *100 }

#Genero descripcion Nivel Tecnológico
planteos <- planteos %>%
  mutate(descripcion_nivel = case_when(nivel == 1 ~ "Alto",
                                       nivel == 2 ~ "Medio",
                                       nivel == 3 ~ "Bajo")) %>% 
  pivot_wider(names_from = descripcion,values_from = promedio) %>% 
  group_by(id_campaña,id_zona,id_grano) %>%
  mutate(Adopcion_NT = escalar(`Adopción de NT`))
...
```{r, fig.height=9, fig.width=9, echo=TRUE}
#Grafico
planteos %>% filter(descripcion_grano=="Maíz 1° Temprano")  %>%  #filtro cultivo
ggplot(.,mapping = aes(x = descripcion_campania,
                       y = Urea, group=nivel,color = as.factor(descripcion_nivel))) +
